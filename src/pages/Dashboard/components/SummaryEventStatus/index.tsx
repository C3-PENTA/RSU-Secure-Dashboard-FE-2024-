...
function SummaryEventStatus(props: SummaryEventStatusProp) {
  ...
  const rows = elements.map((element, index) => (
    <React.Fragment key={index}>
      ...
      <tr
        key={element.device + '2'}
        className={cx(classes.cells, { [classes.noTopBorder]: element.device !== 'RF Scanner' })}
      >
        <td className={cx(classes.cells)} style={{ borderTop: 'none' }}>
          통신
        </td>
        <td className={cx(classes.cells)} style={{ borderTop: 'none' }}>
          {element.normalCommunication}
        </td>
        <td className={cx(classes.cells)} style={{ borderTop: 'none' }}>
          {element.errorCommunication}
        </td>
        <td className={cx(classes.cells)} style={{ borderTop: 'none' }}>
          {element.totalCommunication}
        </td>
      </tr>
    </React.Fragment>
  ));
